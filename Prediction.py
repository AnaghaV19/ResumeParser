...
def process():
    ...
    #print("resumeDataSet['cleaned_resume']======",resumeDataSet['cleaned_resume'])
    var_mod = ['Category']
    le = LabelEncoder()
    for i in var_mod:
    ...
    print("Classess",le.classes_)
    resultclass=[]
    resumeDataSet = pd.read_csv('testes.csv' ,encoding='utf-8')
    resumeDataSet['cleaned_resume'] = resumeDataSet.Resume.apply(lambda x: cleanResume(x))
    requiredText = resumeDataSet['cleaned_resume'].values
    #word_vectorizer.fit(requiredText)
    WordFeatures = word_vectorizer.transform(requiredText)
    ...
    for i in range(len(le.classes_)):
        resultclass.append(le.classes_[i])
